...
from pymclevel import alphaMaterials, MCSchematic, MCLevel, BoundingBox, MCInfdevOldLevel
...
from editortools.select import NudgeSelectionOperation
...
inputs = [
			(
				...
				("Initial Seed                          ", 0),
				("Final Seed                          ", 5),
				("Number of structures  ", 10),				# I am trying to give randomness
				("Number of bridges      ", 3),
...
# level is an MCInfdevOldLevel or MCLevel object
# box is an BoundingBox object
def perform(level, box, options):
	...
	# code added from Aldo Fumagalli
	#seed = options["Seed                          "]
	number_of_bridges = options["Number of bridges      "]
	number_of_structures = random.randint(options["Number of structures  "], 20)		#randomness between 10 and 20
	...
	# code added from Aldo Fumagalli
	for epoch in range (options["Initial Seed                          "], options["Final Seed                          "]):
		seed = epoch
		...
		schematic.saveToFile("/Users/ikros/Desktop/outputs/"+ str(epoch) +".schematic")
		level.close()